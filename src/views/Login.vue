...
function login() {
  ...
  // 调用后端登录API（通过 Vite 代理到 http://localhost:8080）
  ...
  .then(data => {
    // 登录成功，存储后端返回的用户信息和 Token
    const user = data.user || {};
    localStorage.setItem('username', user.username || loginData.username);
    localStorage.setItem('role', user.role || 'student');
    // 存储组织信息，供前端按党支部/小组筛选
    localStorage.setItem('branch', user.branch || 'all');
    localStorage.setItem('groupName', user.group_name || 'all');
